...
class ReportExportController extends Controller
{
    ...
    public function actionPdfTuan1()
    {
        ...
        // Render template
        $html = file_get_contents(Yii::getAlias('@app/modules/work_assignment/views/report/default/pdf-tuan.php'));
        $html = strtr($html, [
            '${data}' => $data,
            '${tu_ngay}' => $tu_ngay,
            '${den_ngay}' => $den_ngay,
    ...
    public function actionPdf()
    {
        ...
        // Render template
        $html = file_get_contents(Yii::getAlias('@app/modules/work_assignment/views/report/default/pdf.php'));
        $html = strtr($html, [
            '${data}' => $data,
            '${tu_ngay}' => $tu_ngay,
            '${den_ngay}' => $den_ngay,
...
public function actionWordTuan()
{
    ...
    // 1️⃣ Lấy dữ liệu
    $searchModel = new \app\modules\work_assignment\models\KpiWorkAssignmentSearch();
    $postData = Yii::$app->request->post();
    $dataProvider = $searchModel->search($postData ?: Yii::$app->request->queryParams);
    $dataProvider->sort = [
        'defaultOrder' => ['start_date' => SORT_ASC]
    ...
    // 2️⃣ Load template
    $templatePath = Yii::getAlias('@app/modules/work_assignment/views/report/default/template-word.docx');
...
{
    $templateFile = Yii::getAlias('@app/modules/work_assignment/views/report/default/template-excel.xlsx');
