                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            <?php
								...
								$departments = DepartmentForm::find()->all();
								$data = [];
								foreach ($departments as $index => $dept) {
									$data[$dept->department_id] = '<div style="display: flex;">
                        ...
                        <div class="col-md-6 col-sm-12">
                           <?php
								...
								foreach ($items as $index => $dept) {
									$data[$dept->position_id] = '<div style="display: flex;">
															<div style="width:24px;">'.($index + 1).'</div> 
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">                          
							<?php
								...
								$data = [];
								foreach ($items as $index => $dept) {
									$data[$dept->business_field_id] = '<div style="display: flex;">
															<div style="width:24px;">'.($index + 1).'</div> 
															<div>'.$dept->name.'</div>
														</div>';
								}
...
.Staff-form-form .card-header-info {
...
.Staff-form-form .card-header-account {
...
/* ===== Card Body gọn ===== */
.Staff-form-form .card-body {
    padding: 0rem 0.75rem;     /* giảm padding */
}

/* First Row trong card */
.Staff-form-form .card-body .card-body-row-top {
	padding-top: 1rem;
}

/* Row trong card */
.Staff-form-form .card-body .row {
...
/* $script = <<< JS
    ...
    statusRange.addEventListener('input', function() {
        ...
        document.getElementById('Staffform-status').value = value;
    });
